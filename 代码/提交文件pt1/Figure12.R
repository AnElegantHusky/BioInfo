...
x = as.data.frame(datas[which(k==2),])
x$cluster = "color"
...
p = plotSM(data = logSoy, dataMetrics = scatMatMetrics, threshVar = "FDR", threshVal = 0.05, pointColor = colList[2+1], saveFile = FALSE)

p[["N_P"]] + xlab("Logged Count") + ylab("Logged Count") + ggtitle(paste("Cluster ", 2, " Significant Genes (n=", format(nGenes, big.mark=",", scientific=FALSE), ")",sep=""))
