...
fulls <- fulls[,c(7,1:6)]
fulls <- fulls[which(fulls$ID %in% x$ID),]
...
ret <- plotSM(fulls, scatMatMetrics, threshVar = "FDR", threshVal = 0.05, pointColor = colList[j], saveFile = FALSE)

ret[["K_L"]] + xlab("Standardized Count") + ylab("Standardized Count") + ggtitle(paste("Cluster ", j, " Significant Added Genes (n=", format(nGenes, big.mark=",", scientific=FALSE), ")",sep="")) + theme(plot.title = element_text(hjust = 0.5, size=11), axis.text=element_text(size=11), axis.title=element_text(size=12), strip.text = element_text(size = 10))
